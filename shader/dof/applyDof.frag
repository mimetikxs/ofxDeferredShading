#version 400

in vec2 vTexCoord;
out vec4 outputColor;

uniform sampler2DRect tex;
uniform sampler2DRect midBlur;
uniform sampler2DRect largeBlur;
uniform sampler2DRect normalAndDepthTex;

uniform float foculRangeStart;
uniform float foculRangeEnd;
uniform float nearEndCoc;
uniform float farEndCoc;

vec4 getSmallBlur(in sampler2DRect t, vec2 uv) {

    vec4 sum = vec4(0.);
    const float weight = 4.0 / 16.;
    sum += weight * texture(t, uv + vec2(0.5, -.5) );
    sum += weight * texture(t, uv + vec2(-.5, -0.5) );
    sum += weight * texture(t, uv + vec2(-0.5, .5) );
    sum += weight * texture(t, uv + vec2(.5, 0.5) );
    return sum;
}

vec4 interpolateDof(vec3 noBlur, vec3 small, vec3 mid, vec3 large, float coc) {
    vec3 c = vec3(0.);

    const float d0 = 0.2;
    const float d1 = 0.4;
    const float d2 = 0.4;

    if (coc < d0) {
        c = mix(noBlur, mid, coc / d0);
        //c = mix(small, mid, coc / d0);
    } else if (coc < d0 + d1) {
        c = mix(mid, large, (coc - d0) / d1);
    } else {
        c = large;
    }

    return vec4(c, 1.);
}

void main() {

    vec4 noBlur = texture(tex, vTexCoord);
    vec4 small = getSmallBlur(tex, vTexCoord);
    vec4 mid = getSmallBlur(midBlur, vTexCoord);
    vec4 large = texture(largeBlur, vTexCoord / 4.);

    float nearCoc = large.a;
    float coc = 0.;
    vec4 col = vec4(0.);

    float depth = texture(normalAndDepthTex, vTexCoord).a;
    if (depth != 0.) {
        float a = farEndCoc / (1. - foculRangeEnd);
        float farCoc = clamp(a * (depth - foculRangeEnd), 0., farEndCoc);
        coc = max(nearCoc, farCoc);
    } else {
        col = noBlur;
        coc = farEndCoc;
    }
    col = interpolateDof(noBlur.rgb, small.rgb, mid.rgb, large.rgb, coc);
    outputColor = col;
    //outputColor = vec4(vec3(coc), 1.);
    //outputColor = texture(tex, vTexCoord);
}
